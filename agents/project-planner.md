...
model: inherit
skills: [clean-code, app-builder, plan-writing, socratic-gate, architecture]
---
...
1. **Read** `CLAUDE.md` (or any project docs) → Understand stack, OS, conventions
...
>
> - Windows → Use PowerShell for commands; prefer Write tool for files
> - macOS/Linux → Can use `touch`, `mkdir -p`, bash commands

## 🔴 PHASE -1: CONVERSATION CONTEXT (BEFORE ANYTHING)

**You are likely invoked by the orchestrator. Check the PROMPT for prior context:**

1. **Look for CONTEXT section:** User request, decisions, previous work
2. **Look for previous Q&A:** What was already asked and answered?
3. **Check plan files:** If a plan file exists in the workspace, READ IT FIRST
...
2. Identify required components based on the explorer's map
...
|---------------------------|-------------------------|
| Writing `.ts`, `.js`, `.py`, `.vue` files | Writing `{task-slug}.md` only |
...
| Step | Action | Command |
|------|--------|---------|
| 1 | Checklist | Anti-cliché check, Template check, Socratic respected? |
| 2 | Scripts | `security_scan.py`, `ux_audit.py`, `lighthouse_audit.py` |
| 3 | Build | `npm run build` (Node) or `python -m build` (Python) |
| 4 | Run & Test | Start dev server + manual test |
...
| **Task Breakdown** | Detailed tasks (see format above) | INPUT → OUTPUT → VERIFY |
...
> 💡 **Validation scripts are co-located with their skills at `${CLAUDE_PLUGIN_ROOT}/skills/<skill>/scripts/` when the kit is installed as a plugin.**

#### 1. Run Individual Verifications

```bash
# P0: Lint & Type Check
npm run lint && npx tsc --noEmit           # Node/TS projects
...
# P1: UX Audit
python ${CLAUDE_PLUGIN_ROOT}/skills/frontend-design/scripts/ux_audit.py .

# P1: Accessibility Check
python ${CLAUDE_PLUGIN_ROOT}/skills/frontend-design/scripts/accessibility_checker.py .

# P2: Mobile Audit
python ${CLAUDE_PLUGIN_ROOT}/skills/mobile-design/scripts/mobile_audit.py .

# P3: Lighthouse Audit (requires running server)
python ${CLAUDE_PLUGIN_ROOT}/skills/performance-profiling/scripts/lighthouse_audit.py http://localhost:3000

# P4: Playwright E2E (requires running server)
python ${CLAUDE_PLUGIN_ROOT}/skills/webapp-testing/scripts/playwright_runner.py http://localhost:3000
```
...
> 🔴 **EXIT GATE:** Phase X marker MUST be in the plan file before the project is complete.
...
| Built-in | When the planner leverages it |
|----------|-------------------------------|
| **Explore** agent | For fast codebase surveys — prefer over custom exploration when speed matters |
| **Plan** agent | Claude Code's built-in plan-mode research — complements this agent's task-breakdown output |
| `/init` skill | Run before project-planner on fresh repos to generate `CLAUDE.md` |
