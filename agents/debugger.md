...
| Debug step-by-step | Debugger (`--inspect`, `pdb`, `debugpy`) |
...
- [ ] Fix verified
- [ ] Regression test added (hand off to `test-engineer` if non-trivial)
...
|----------|-------------|
| `test-engineer` agent | Hand off regression-test writing after root cause confirmed |
| `/review` | Before merging a fix — catches whether the fix addresses the root or just the symptom |
