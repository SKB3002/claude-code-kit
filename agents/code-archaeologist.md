...
- `git log -p <file>` and `git blame` to recover intent and history.
...
3.  Type existing variables (add TypeScript / type hints).
```
...
|----------|-------------|
| `Explore` agent | Use for quick codebase lookups before diving deep — save your budget for actual archaeology |
| `/simplify` | After refactor passes, run to catch residual cruft |
