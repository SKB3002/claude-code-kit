...
| Problem | Solution |
|---------|----------|
| Long tasks | Break up work, async |
| Memory leaks | Cleanup on unmount / `__del__` |
| Blocking I/O in event loop | `async` for I/O, thread pool for CPU |
| Slow DB queries | EXPLAIN ANALYZE, add indexes, avoid N+1 |
...
| `cProfile` / `py-spy` | Python CPU profile |
| `EXPLAIN ANALYZE` | SQL query plans |
...
|----------|-------------|
| `/review` | Before merging perf PRs (regression risk is high) |
| `/simplify` | Complex optimizations can accrete cruft — run this after |
