---
name: instincts
description: Project-scoped learned preferences captured via /kit:instincts promote. Load this skill when an agent needs to consult project-specific conventions that aren't obvious from the code. Never auto-captures — user explicitly approves every entry.
allowed-tools: Read, Glob, Write, Edit, Bash
---

# Instincts — learned project preferences

> **Contract:** instincts are user-authored. No silent capture, no background agent, no LLM fee on idle. An instinct exists only because the user ran `/kit:instincts promote` and approved it. The kit's job is to store them faithfully and make them discoverable — nothing else.
...
These are learned preferences, not doctrine. They shift over time. Codifying them in CLAUDE.md is too heavy; in comments is too scattered. `.kit/instincts.yaml` is the in-between layer: searchable, git-trackable, per-project, and cheap to edit.
...
Do **not** load this skill on every agent dispatch. Loading it means reading the file, which costs context. The calling agent should read `.kit/instincts.yaml` lazily, only when its scouting step flags a relevant domain.
...
| project | `<project-root>/.kit/instincts.yaml`      | **Yes**     | Team-visible preferences for this repo |
| global  | `~/.claude/kit/instincts.yaml`            | No          | Cross-project preferences — user's private machine |

**Why project instincts are git-tracked by default:** team visibility is the feature. A new engineer clones the repo and `/kit:instincts show` reveals the preferences the team has already encoded. If a user wants privacy, they add `.kit/` to `.gitignore` explicitly.
...
1. **Resolve paths.** Check both `<project-root>/.kit/instincts.yaml` and `~/.claude/kit/instincts.yaml`. Read both if present. Project takes precedence on conflict.
...
Confidence is a number 0.0–1.0 assigned by `/kit:instincts promote` based on session evidence:
...
The `last_seen` field exists for the user to eyeball staleness during `/kit:instincts show`, not for automated pruning.

---

## 7. Anti-patterns (never do)

- ❌ Auto-capture without `/kit:instincts promote` approval
- ❌ Dispatch a background Haiku agent to watch tool calls
- ❌ Write to `.kit/instincts.yaml` from anywhere except `/kit:instincts promote`
...
| Need | Go to |
|---|---|
| Inspect or write instincts | [`/kit:instincts`](../../commands/instincts.md) |
