...
> Authoritative reference for `.kit/instincts.yaml` (project scope) and `~/.claude/kit/instincts.yaml` (global scope).
...
| `confidence` | yes | 0.0–1.0 float. Minted by `/kit:instincts promote` from session evidence (see SKILL.md §5). |
...
`/path/to/my-fastapi-app/.kit/instincts.yaml`:
...
## Invariants (validated by `/kit:instincts promote` at write time)
...
If any invariant is violated at read time, `/kit:instincts show` reports the specific entry and refuses to render it — but does not modify the file. User fixes manually.
...
- Don't delete entries you want to keep — use `/kit:instincts clear <id>` or just remove the YAML block
...
If you change the file and it breaks YAML, `/kit:instincts show` will tell you which line. No silent corruption.
